...
def main(args):
    ...
    if args.fast_adamax:
        ...
        cnn_optimizer = torch.optim.Adamax(model.parameters(), args.learning_rate,
                                           weight_decay=args.weight_decay, eps=1e-3)
    optimizer_D = torch.optim.Adam(discriminator.parameters(), lr=args.learning_rate, betas=(5e-1,0.999))
    ...
    checkpoint_D_file = os.path.join(args.save, 'checkpoint_D.pt')
    if args.cont_training:
        ...
        checkpoint_D = torch.load(checkpoint_D_file, map_location='cpu')
    ...
    for epoch in range(init_epoch, args.epochs):
        ...
        train_nelbo, global_step = train(train_queue, model, discriminator, cnn_optimizer, optimizer_D, grad_scalar, global_step, warmup_iters, writer, logging)
        logging.info('train_nelbo %f', train_nelbo)
        writer.add_scalar('train/nelbo', train_nelbo, global_step)
        
        valid_neg_log_p, valid_nelbo = test(valid_queue,model,discriminator, num_samples=10, args=args, logging=logging)
        ...
        if epoch % save_freq == 0 or epoch == (args.epochs - 1):
            if args.global_rank == 0:
                ...
                torch.save({'state_dict': discriminator.state_dict(),
                            'optimizer': optimizer_D.state_dict()}, checkpoint_D_file)

    # Final validation
    valid_neg_log_p, valid_nelbo = test(valid_queue, model,discriminator, num_samples=1000, args=args, logging=logging)
...
def train(train_queue, model, discriminator, cnn_optimizer, optimizer_D, grad_scalar, global_step, warmup_iters, writer, logging):
    ...
    for step, x in enumerate(train_queue):
        ...
        fake_aux_gt = Variable(LongTensor(args.batch_size).fill_(10), requires_grad=False)
        ...
        # fake_img = create_generator_vae(model, args.batch_size, 1)
        
        with torch.no_grad():
            logits = model.sample(args.batch_size, 1.0)
            output = model.decoder_output(logits)
            output_img = output.mean if isinstance(output, torch.distributions.bernoulli.Bernoulli) else output.mean()
        
        fake_pred, fake_aux = discriminator(output_img.cuda())
        ...
        if (global_step + 1) % 100 == 0:
            ...
            writer.add_scalar('train/d_acc', d_acc, global_step)
...
def test(valid_queue, model,discriminator, num_samples, args, logging):
    if args.distributed:
        dist.barrier()
    ...
    for step, x in enumerate(valid_queue):
        ...
        writer.add_scalar('val/d_loss', d_loss, step)
        writer.add_scalar('val/d_acc', d_acc, step)
